#!/usr/bin/env python3
"""
Otimizador para Bytecode - Trabalho Prático Compiladores
Remove instruções redundantes sem alterar a semântica do programa
"""
...
class BytecodeOptimizer:
    """Otimizador para código bytecode"""
    ...
    def optimize_push_pop(self) -> int:
        """Remove sequências PUSH seguidas imediatamente de POP"""
        optimized = []
        i = 0
        removed_count = 0
        ...
        while i < len(self.instructions):
            ...
            if i + 1 < len(self.instructions):
                ...
                # Verifica se é PUSH seguido de POP
                if current.startswith("PUSH ") and next_instr == "POP":
                    # Remove ambas as instruções
                    removed_count += 2
                    i += 2
                    continue
    ...
    def optimize_redundant_loads(self) -> int:
        """Remove LOADs redundantes da mesma variável"""
        optimized = []
        i = 0
        removed_count = 0
        ...
        while i < len(self.instructions):
            ...
            if current.startswith("LOAD "):
                ...
                # Procura por LOADs consecutivos da mesma variável
                j = i + 1
                consecutive_loads = 0
                ...
                while j < len(self.instructions):
                    ...
                    # Para se encontrar uma instrução que modifica a pilha
                    # ou se não é um LOAD da mesma variável
                    if (
                        next_instr == ""
                ...
                # Adiciona apenas o primeiro LOAD
                optimized.append(self.instructions[i])

                # Pula os LOADs redundantes
                if consecutive_loads > 0:
                    # Adiciona DUPs para manter a semântica
    ...
    def optimize_dead_code(self) -> int:
        """Remove código morto após HALT, RET ou JMP incondicional"""
        optimized = []
        removed_count = 0
        ...
        for instruction in self.instructions:
            ...
            # Se estamos pulando código morto
            if skip_until_label:
                if instr.endswith(":"):  # Encontrou um label
                    skip_until_label = False
                    optimized.append(instruction)
                else:
                    if instr and not instr.startswith("#"):
                        removed_count += 1
                    # Pula a instrução (código morto)
                continue

            optimized.append(instruction)

            # Verifica se próximas instruções são código morto
    ...
    def optimize_constant_folding(self) -> int:
        """Otimização de folding de constantes para operações simples"""
        optimized = []
        i = 0
        removed_count = 0
        ...
        while i < len(self.instructions):
            ...
            # Verifica padrão: PUSH a, PUSH b, ADD
                ...
                if next1.startswith("PUSH ") and next2 in [
                    ...
                    try:
                        ...
                        if next2 == "ADD":
                            ...
                            # Não pode otimizar (divisão por zero)
                            optimized.append(self.instructions[i])
                            i += 1
                            continue

                        # Substitui por uma única instrução PUSH
                        optimized.append(f"PUSH {result}")
                        removed_count += 2  # Removeu 2 instruções
                        i += 3
                        continue
    ...
    def optimize(self) -> Tuple[str, dict]:
        """Aplica todas as otimizações e retorna código otimizado"""
        stats = {
            "push_pop_removed": 0,
